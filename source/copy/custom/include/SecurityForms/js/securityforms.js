...
lab321.sform.disableForm = function(formSelector, enabledFields) {
    ...
    $(formSelector).find('select:visible:not(.sf-disabled), input:not(.button), textarea, .remove_item_button').each(function() {
...
lab321.sform.disableElement = function(elem) {
    ...
    if(x.is(':checkbox') || x.attr('type') == 'button')
        x.attr('disabled', 'disabled');
    x.parent('td').find('button').attr('disabled', 'disabled'); //$('#btn_'+elem.id+', #btn_clr_'+elem.id)
    ...
    if(x.hasClass('remove_item_button'))
        x.hide();
}
...
lab321.sform.enableForm = function(formSelector, disabledFields) {
    ...
    $(formSelector).find('select.sf-disabled, input.sf-disabled:not(.button), textarea.sf-disabled, .remove_item_button.sf-disabled').each(function() {
        for(var i in disabledFields)
            if(disabledFields[i] == this.name || (!this.name && (disabledFields[i]+'_date' == this.id || disabledFields[i]+'_hours' == this.id || disabledFields[i]+'_minutes' == this.id || disabledFields[i]+'_meridiem' == this.id)))
                return;
        lab321.sform.enableElement(this);
    });
}

lab321.sform.enableElement = function(elem) {
    var x = $(elem);
    $('img#'+elem.id+'_trigger.sf-dummy-trigger').remove();
    $('img#'+elem.id+'_trigger').show();
    //if(x.is(':checkbox'))
        x.removeAttr('disabled');
    x.parent('td').find('button').removeAttr('disabled');
    if(x.is('select')) {
        x.show();
        x.next('.sf-select').remove();
    }
...
<table>\
  <tr>\
    ...
    .end()
    .prependTo($(formSelector+' #EditView_tabs').length ? $(formSelector+' #EditView_tabs').first() : $(formSelector).first());
}
